...
export function buildTutorialSteps(): TutorialStep[] {
  ...
  return [
    // ── Акт 0. Приветствие ────────────────────────────────────────────────────
    {
      ...
      text: [
        'Следоку — это судоку в детективной обёртке. Здесь не цифры, а люди: вы — сотрудник детективного агентства, и вам предстоит раскрыть своё первое дело.',
    ...
    },
    {
      ...
      text: [
        'Некоторые предметы можно занять: на стульях можно сидеть — как эти три на карте. В других делах бывают и диваны, кресла, кровати и даже лошади.',
        'Такие предметы не мешают дедукции: клетка со стулом — обычная клетка, на которую можно поставить человека.',
      ],
      target: {
        kind: 'cells',
    ...
    // ── Акт 2. Борис: метки + урок B (пересечения) ────────────────────────────
    {
      ...
      text: [
        ...
        'Угол — это клетка, у которой сходятся две стены одной зоны. Стульёв на карте три, но в углу своей зоны стоят два: в кабинете и в приёмной. Который из них — пока неизвестно.',
      ],
    ...
    },
    {
      ...
      text: [
        ...
        'Клетки на пересечениях этих линий выбивают обе метки сразу: если бы Борис встал туда, обе метки оказались бы не у дел — а других вариантов у него нет. Значит, на пересечениях точно никого не будет.',
    ...
    },
    {
      ...
      text: ['Возьмите Андрея в список — будем размечать его кандидатов.'],
      target: { kind: 'selector', selector: sel('roster-person-andrei') },
      advance: { kind: 'selectPerson', personId: 'andrei' },
      tooltipSide: 'left',
    },
    {
      ...
      title: 'Метки Андрея',
      text: [
        'Поставьте метки на все три свободные клетки первого ряда: левый угол, клетка рядом с доской для записей и правый край.',
      ],
      target: { kind: 'cells', cellIds: [cellId(0, 0), cellId(0, 1), cellId(0, 4)] },
    ...
    },
    {
      ...
      text: [
        'У Андрея аж три кандидата. Не пугайтесь: метки для того и нужны, чтобы фиксировать неопределённость.',
    ...
    },
    {
      ...
      text: ['Андрей — первый ряд, клетка рядом с доской для записей.'],
      target: { kind: 'cell', cellId: cellId(0, 1) },
      advance: { kind: 'place', personId: 'andrei', cellId: cellId(0, 1) },
      alsoSelectors: [sel('roster-person-andrei')],
      tooltipSide: 'bottom',
